...
assetView = (rackId, optId) => {
    ...
    this.setState({
        rackID: rackId, error: false, message: "",
        assetMacId: "", details: [0, 0, 0]
    });
        ...
        .then((response) => {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                if (data.length !== 0) {
                    ...
                    this.setState({
                        detailsKey: "assets",
                        details: [detAsset.assetcount, detAsset.rackcapacity,
                        detAsset.available],
                    })
                }
            }
...
assetServerDetails = (tagid) => {
    ...
    axios({ method: 'GET', url: "/api/track?id=" + tagid })
        .then((response) => {
            ...
            if (response.status === 200 || response.status === 201) {
                if (data.length !== 0) {
                    let utilisation = [], tempf = [], tempb = [], humidityf = [],
                        humidityb = [], energy = [], power = [],cate=[];
                    for (let i = 0; i < data.length; i++) {
                        let utiData = [], tempfData = [], tempbData = [], humifData = [],
                            humibData = [], energyData = [], powerData = [];
...
thermalView = (rackId, optId) => {
    ...
    this.setState({ rackID: rackId, series: [], series1: [], assetMacId: "", details: [0, 0, 0] });
    $("#popup").css("display", "block");
    $("#asset_graphBlock").css("display", "none");
        ...
        .then((response) => {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                if (data.length !== 0) {
                    this.setState({
                        detailsKey: "thermal",
                        details: [detThermal.maxtemp, detThermal.mintemp, detThermal.avgtemp],
                    })
                    for (let i = 0; i < data.length; i++) {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                if (data.length !== 0) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        ...
                        else if (energy >= 11 && energy <= 15) clr = "rgba(255,125,0,0.6)";
...
imgMoveLeft = () => {
  ...
  render() {
    const { error, message, rackID,
      assetMacId, series, series1,
      detailsKey, details, options1 } = this.state;
    return (
      <div
      style={
        {
                        ...
                        >
                            ...
                            <div id="thermal_det" style={{
                                ...
                                <div style={{ display: "flex", margin: "10px 20px" }}>
                                    ...
                                    <p style={{ marginBottom: "0px" }}>Optimum (21&deg;C - 34&deg;C)</p>
                                </div>
                            ...
                            <div id="asset_graphBlock"
                                ...
                                <h5 style={{ marginLeft: "20px" }}>AssetID : 5a-c2-15-00-00-01</h5>
                                {
                                    series.length ? (
                                        <div>
                                            <div>
                                                <div id="chart">
                                                    <div id="chart-timeline">
                                                        <ApexCharts options={this.state.options}
                                                            series={series} type="area" height={350} />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    series1.length ? (
                                        <div>
                                            <div>
                                                <div id="chart">
                                                    <div id="chart-timeline">
                                                        <ApexCharts options={this.state.options1}
                                                            series={series1} type="area" height={350} />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
